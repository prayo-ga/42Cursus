...
/*   Updated: 2024/04/16 13:12:57 by prayo-ga         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_textures(t_game *game)
{
	...
	game->textures->ground = mlx_load_png("assets/background/ground.png");
	game->textures->wall = mlx_load_png("assets/background/bush.png");
	game->textures->bone = mlx_load_png("assets/win/bone.png");
	game->textures->character = mlx_load_png("assets/character/character.png");
	game->textures->close_chest = mlx_load_png("assets/win/close_chest.png");
	game->textures->open_chest = mlx_load_png("assets/win/open_chest.png");
}
...
void	get_images(t_game *game)
{
	...
	game->images->character = mlx_texture_to_image(game->mlx,
			game->textures->character);
	game->images->close_chest = mlx_texture_to_image(game->mlx,
			game->textures->close_chest);
	game->images->open_chest = mlx_texture_to_image(game->mlx,
			game->textures->open_chest);
...
void	draw_ground(t_game *game, t_images *image)
{
	...
	while (game->map[oy])
	{
		...
		while (game->map[oy][ox])
		{
			...
			{
				mlx_image_to_window(game->mlx, image->open_chest, ox * 32, oy
					* 32);
				mlx_image_to_window(game->mlx, image->close_chest, ox * 32, oy
					* 32);
			}
			ox++;
		}
	...
	while (game->map[oy])
	{
		...
		while (game->map[oy][ox])
		{
			...
			if (game->map[oy][ox] == START_PLAYER)
				mlx_image_to_window(game->mlx, images->character, ox * 32, oy
					* 32);
			ox++;
		}
		oy++;
